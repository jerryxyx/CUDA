...
__global__ void HestonPowerOption(
	bool isCall,
	double nPower,
	double rate, 
	//double volatility,
	...
	float *d_price, float *d_stockPrice,
	curandState_t *d_state,
	curandState_t *d_state1, curandState_t *d_state2)
{
	int tid = threadIdx.x + blockDim.x * blockIdx.x;
	double dt = timeToMature / double(n_iters);
	double logStockValue = log(initialPrice);
	...
	if (tid < size)
	{
		...
		for (int i = 0; i < n_iters; i++)
		{
			...
			Variance = Variance + kappa*(meanVariance - Variance)*dt + volSigma*sqrt(Variance*dt)*curand_normal(&d_state1[tid]);
			Variance = abs(Variance);
			//Heston
			logStockValue += (rate - 0.5*Variance)*dt;
			logStockValue += sqrt(dt*Variance)*(rho*curand_normal(&d_state1[tid]) + sqrt(1 - rho*rho)*curand_normal(&d_state2[tid]));
			//Black Scholes
			//logStockValue = logStockValue + (rate - 0.5*meanVariance)*dt + sqrt(meanVariance*dt)*curand_normal(&d_state1[tid]);
		}
		d_stockPrice[tid] = exp(logStockValue);//stock price at expiration
		if (isCall)
			optionValue = exp(nPower*logStockValue)-strikePrice;
		else
			optionValue = strikePrice - exp(nPower*logStockValue);
		if (optionValue < 0)
			optionValue = 0;
		d_price[tid] = optionValue;//option value at expiration
	}
...
int main()
{
	...
	float nPower, rate, volatility, timeToMature, n_iters, stockPrice, exercisePrice,initialVariance,meanVariance,kappa,varSigma,rho;
	...
	std::cin >> optionType;
	std::cout << "The order of power option? (1,2,3... not restricted in integer)" << std::endl;
	std::cin >> nPower;
	...
	init << < (n_trials + blockSize - 1) / blockSize, blockSize >> >(time(nullptr), d_state1);
	init << < (n_trials + blockSize - 1) / blockSize, blockSize >> >(time(nullptr), d_state2);
...
//		d_state);
	HestonPowerOption << <(n_trials + blockSize - 1) / blockSize, blockSize >> >(
		isCall,
		nPower,
		rate, 
		//volatility,
	...
	double meanOptionValue = 0;
	double stdOptionValue = 0;
	...
	double meanStockPrice = 0;
	double stdStockPrice = 0;
	for (int i = 0; i < n_trials; i++) {
	...
	int zeroCounter = 0;
	double meanLogStockPrice = 0;
	double stdLogStockPrice = 0;
	for (int i = 0; i < n_trials; i++) {
